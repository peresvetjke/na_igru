class Player < ApplicationRecord
  ...
  has_many :favorite_locations, through: :player_locations, source: :location
  ...
  scope :players_with_favorite_location,  -> (location) { joins(:player_locations).where('location_id = ?', location.id) }
  ...
  def player_location(location)
    PlayerLocation.where(location: location, player: self).first
  end
